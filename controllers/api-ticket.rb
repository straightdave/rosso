...
post '/api/tickets' do
  ...
  # NOTE: we don't check access when requesting TGT
  # that't ST issuer's responsibility
  #halt 402, "err: user has no access"   unless user.has_access?(@svc)
  ...
  if stored_tgt = @redis.get "U-#{user.id}"
    ...
    }.to_json, { ex: settings.tgt_expire }
  end

  ['201', {'Location' => "#{request.path}/#{tgt}", 'Content-Type' => 'text/html'}, [tgt]]
end

# GET ST: this API represents ST Issuer role
#
# request for an ST from rosso with TGT
...
post '/api/tickets/:tgt' do |tgt|
  ...
  halt 401, "err: invalid user"       unless user = User.find_by(id: tgt_items["user_id"])
  ...
  if st_list.size > 0 && (this_st = st_list.select! {|x| x.start_with?(st_prefix) })
    ...
    }.to_json, { ex: settings.tgt_expire }    # reset expiration
